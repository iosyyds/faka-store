...
export default function Icon({ name, size = 16, color, style, className }: { name: string; size?: number; color?: string; style?: React.CSSProperties; className?: string }) {
  ...
  if (!icon) return null;
  return <span className={className} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: size, height: size, color: color || 'currentColor', ...style }}>{icon}</span>;
}
