...
export default function OrderModal({
  ...
  const currentPrice = product.has_spec && product.specs?.length
    ? product.specs.find((s) => s.id === specId)?.price || product.price
    : product.price;
  const currentStock = product.has_spec && product.specs?.length
    ? product.specs.find((s) => s.id === specId)?.stock || 0
    : product.stock;
  const maxQty = Math.min(quantity, currentStock, 10);
  ...
  async function handleSubmit() {
    if (!contact.trim()) { setErr("请输入联系方式（手机号/邮箱/QQ）"); return; }
    ...
    try {
      ...
      setQueryPwd(res.query_pwd);
      setQrCode(res.qr_code);
      setTotalAmount(res.total_amount);
      setCountdown(res.pay_timeout * 60);
      setStep("pay");
      startPolling(res.order_no, res.query_pwd);
      startCountdown();
    } catch (e: any) {
      ...
      <div className="modal-box" style={{ padding: 24 }}>
        ...
        <div style={{ background: "#f9fafb", borderRadius: 10, padding: 14, marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 13, color: "#6b7280" }}>
            {product.has_spec && product.specs?.find((s) => s.id === specId)
              ? `规格：${product.specs.find((s) => s.id === specId)?.spec_name}`
              : "自动发货 · 秒到账"}
          </div>
        </div>
                  ...
                  {product.specs.map((s) => (
                    <button
                      ...
                      style={{
                        ...
                        borderColor: specId === s.id ? "#4f46e5" : "#e5e7eb",
                        background: specId === s.id ? "#eef2ff" : "#fff",
                        color: specId === s.id ? "#4f46e5" : "#374151",
        ...
        {step === "pay" && (
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontSize: 13, color: "#6b7280", marginBottom: 16 }}>
              订单号：{orderNo} · 剩余 {Math.floor(countdown / 60)}:{String(countdown % 60).padStart(2, "0")}
            </div>
            {qrCode ? (
              <div style={{ display: "inline-block", padding: 12, background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12 }}>
                <img src={qrCode} alt="支付二维码" style={{ width: 200, height: 200, display: "block" }} />
              </div>
            ) : (
        ...
        {step === "success" && order && (
          <div>
            ...
            {product.tutorial && (
              <div style={{ background: "#fef9c3", padding: 10, borderRadius: 8, fontSize: 13, marginTop: 10 }}>
                <strong>使用教程：</strong>{product.tutorial}
              </div>
            )}
...
const qtyBtn: React.CSSProperties = {
  ...
  background: "#fff", cursor: "pointer", fontSize: 16, fontWeight: 600, color: "#374151",
};
