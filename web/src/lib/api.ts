...
export function createOrder(data: { product_id: number; spec_id?: number; quantity: number; contact: string }) {
  return req<CreateOrderResult>("order_create.php", undefined, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
  });
}
export function queryOrder(order_no: string, pwd: string) {
  return req<OrderInfo>("order_query.php", { order_no, pwd });
}
export function fetchOrderList(contact: string, pwd: string) {
  return req<OrderInfo[]>("order_list.php", { contact, pwd });
}
...
export function submitAfterSale(data: { order_no: string; contact: string; content: string }) {
  return req<{ id: number }>("after_sale.php", undefined, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
  });
