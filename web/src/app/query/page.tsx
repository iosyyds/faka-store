...
export default function QueryPage() {
  ...
  async function handleQuery() {
    ...
    try {
      const o = await queryOrder(orderNo.trim(), pwd.trim());
      setOrder(o);
  ...
  async function handleList() {
    ...
    try {
      const list = await fetchOrderList(contact.trim(), listPwd.trim());
      setOrders(list);
    } catch (e: any) { setErr(e.message || "查询失败"); }
    finally { setLoading(false); }
  }
  ...
  const statusMap: Record<number, { label: string; cls: string }> = {
    0: { label: "待支付", cls: "tag-yellow" },
    1: { label: "已完成", cls: "tag-green" },
    2: { label: "已取消", cls: "tag-gray" },
    3: { label: "已退款", cls: "tag-red" },
  };
        ...
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e8eaed", padding: 24 }}>
          ...
          {tab === "single" ? (
            <>
              <div className="form-group">
                ...
                <input className="form-input" placeholder="如 FK20260904xxxxxx" value={orderNo} onChange={(e) => setOrderNo(e.target.value)} />
              </div>
              <div className="form-group">
                <label className="form-label">查询密码</label>
                <input className="form-input" type="password" placeholder="下单时生成的6位密码" value={pwd} onChange={(e) => setPwd(e.target.value)} />
              </div>
              <button className="btn btn-primary" style={{ width: "100%", padding: 12 }} onClick={handleQuery} disabled={loading}>
                {loading ? "查询中..." : "立即查询"}
              </button>
            </>
          ) : (
            <>
              <div className="form-group">
                <label className="form-label">联系方式（下单时填写的）</label>
                <input className="form-input" placeholder="手机号 / 邮箱 / QQ" value={contact} onChange={(e) => setContact(e.target.value)} />
              </div>
              <div className="form-group">
                <label className="form-label">查询密码</label>
                <input className="form-input" type="password" placeholder="任意一笔订单的查询密码" value={listPwd} onChange={(e) => setListPwd(e.target.value)} />
              </div>
              <button className="btn btn-primary" style={{ width: "100%", padding: 12 }} onClick={handleList} disabled={loading}>
                {loading ? "查询中..." : "查看全部订单"}
              </button>
            </>
          )}
          ...
          {order && (
            <div style={{ marginTop: 20, padding: 16, background: "#f9fafb", borderRadius: 10 }}>
              ...
              {order.status === 1 && order.cards.map((c, i) => (
            ...
            <div style={{ marginTop: 20 }}>
              ...
              {orders.map((o) => (
                <div key={o.order_no} style={{ padding: 14, background: "#f9fafb", borderRadius: 10, marginBottom: 10 }}>
                  ...
                  {o.status === 1 && o.cards.map((c, i) => (
                    <div key={i} className="card-secret" style={{ marginTop: 8 }}>
                      <div style={{ fontSize: 11, color: "#6b7280" }}>卡密{i + 1}</div>
                      {c.content}
                      <button className="copy-btn" onClick={() => copy(c.content)}>复制</button>
                    </div>
                  ))}
          ...
          {tab === "list" && !loading && orders.length === 0 && !err && (
            <div className="empty" style={{ padding: "30px 0" }}><div className="empty-icon">📭</div><div>暂无订单</div></div>
          )}
        </div>
      </main>
