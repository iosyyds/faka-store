...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchSite(), fetchCategories()])
      .then(([s, c]) => {
        setSite(s);
        setCats(c);
  ...
  useEffect(() => {
    ...
    fetchProducts({
      category_id: activeCat ? String(activeCat) : "",
      keyword: urlKw,
      sort,
    })
  ...
  return (
    <>
      ...
      {/* 公告弹窗 */}
      {showAnnouncement && site?.announcements?.find((a) => a.is_popup === 1) && (
        <div className="modal-mask" onClick={(e) => { if (e.target === e.currentTarget) setShowAnnouncement(false); }}>
          <div className="modal-box" style={{ padding: 24, maxWidth: 400 }}>
            <h3 style={{ margin: "0 0 12px", fontSize: 17 }}>📢 {site.announcements.find((a) => a.is_popup === 1)?.title}</h3>
            <p style={{ fontSize: 14, color: "#4b5563", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>
              {site.announcements.find((a) => a.is_popup === 1)?.content}
            </p>
      ...
      <main className="container">
        ...
        {site?.announcements?.[0] && !site.announcements[0].is_popup && (
          <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 10, padding: "10px 16px", marginBottom: 20, fontSize: 13, color: "#1d4ed8" }}>
            📢 {site.announcements[0].title}：{site.announcements[0].content}
          </div>
        )}
        ...
        <div className="cat-bar">
          ...
          {cats.map((c) => (
            <span key={c.id} className={`cat-chip ${activeCat === c.id ? "active" : ""}`} onClick={() => setActiveCat(c.id)}>
              {c.icon} {c.name}
            </span>
          ))}
        ...
        {loading ? (
          ...
          <div className="empty">
            ...
            <div>商品加载失败：{err}</div>
            <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 8 }}>请检查后端接口是否正常</div>
          </div>
        ) : products.length === 0 ? (
          <div className="empty">
            <div className="empty-icon">📦</div>
      ...
      <Footer
        siteName={siteName}
        customerService={site?.customer_service}
        qqGroup={site?.qq_group}
