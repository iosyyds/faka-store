      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(255,255,255,0.8)', backdropFilter: 'saturate(180%) blur(20px)', WebkitBackdropFilter: 'saturate(180%) blur(20px)', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
        <div style={{ maxWidth: 980, margin: '0 auto', padding: isMobile ? '0 16px' : '0 22px', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', height: '100%' }} onClick={goHome}>
            ...
            <span style={{ fontSize: 17, fontWeight: 600, lineHeight: '20px', display: 'flex', alignItems: 'center', marginTop: isMobile ? 0 : 7 }}>{site.site_name || '甜甜发卡'}</span>
          </div>
          ...
          {filteredProducts.map((p) => {
            ...
            <div key={p.id} onClick={() => { if (isSoldOut) { showToast('该商品已缺货'); } else { openBuy(p); } }} onMouseEnter={() => !isSoldOut && setHoveredProduct(p.id)} onMouseLeave={() => setHoveredProduct(null)} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: isMobile ? 4 : 6 }}>
                ...
                <div style={{ fontSize: isMobile ? 14 : 15, fontWeight: 600, marginBottom: isMobile ? 3 : 6, overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: isMobile ? 1 : 2, WebkitBoxOrient: 'vertical', lineHeight: 1.4, color: p.stock === 0 ? '#86868B' : '#1d1d1f' }}>{p.name}</div>
                {!isMobile && p.description && <div style={{ fontSize: 12, color: '#86868B', marginBottom: 8, overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 1, WebkitBoxOrient: 'vertical', lineHeight: 1.4 }}>{p.description}</div>}
