...
export default function AfterSalePage() {
  ...
  async function handleSubmit() {
    ...
    try {
      await submitAfterSale({ order_no: orderNo.trim(), contact: contact.trim(), content: content.trim() });
      setSuccess(true);
      ...
      <main className="container" style={{ maxWidth: 560 }}>
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e8eaed", padding: 24 }}>
          ...
          <div className="form-group">
            ...
            <input className="form-input" placeholder="FK开头的订单号" value={orderNo} onChange={(e) => setOrderNo(e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">联系方式 *</label>
            <input className="form-input" placeholder="QQ / 微信 / 手机号，方便客服联系您" value={contact} onChange={(e) => setContact(e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">问题描述 *</label>
            <textarea className="form-input" rows={5} placeholder="请详细描述您遇到的问题..." value={content} onChange={(e) => setContent(e.target.value)} style={{ resize: "vertical" }} />
          </div>
