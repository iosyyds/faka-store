...
export default function FAQPage() {
  ...
  useEffect(() => { fetchFAQs().then((list) => setFaqs(list)).catch(() => {}).finally(() => setLoading(false)); }, []);
      ...
      <main className="container" style={{ maxWidth: 700 }}>
        ...
        {loading ? <div className="spinner" /> : list.map((f) => (
          <div key={f.id} style={{ background: "#fff", border: "1px solid #e8eaed", borderRadius: 10, marginBottom: 10, overflow: "hidden" }}>
            <div style={{ padding: "14px 18px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", fontWeight: 500 }} onClick={() => setOpenId(openId === f.id ? null : f.id)}>
              <span>{f.question}</span>
              <span style={{ color: "#9ca3af", transition: ".2s", transform: openId === f.id ? "rotate(180deg)" : "none" }}>▼</span>
            </div>
            {openId === f.id && (<div style={{ padding: "0 18px 16px", fontSize: 14, color: "#4b5563", lineHeight: 1.8, borderTop: "1px solid #f3f4f6", paddingTop: 12 }}>{f.answer}</div>)}
          </div>
        ))}
      </main>
